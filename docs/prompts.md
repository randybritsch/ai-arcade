...
*   **Diff‑based updates** (so your spec stays current, safely)
...
1.  Create `/docs/project_spec.md` in your repo (empty is fine for now).
2.  Create `/docs/` folder (empty).
3.  Open a new chat and paste **Prompt A**, then **Prompt B**, then **Prompt C**.

***

## Prompt A — Create `project_spec.md` (initial build)
...
Constraints: Performance: fast load, handle ~100–300 submissions smoothly. Security: validate/normalize URLs, basic iframe sandboxing if embedding, no secrets in client. Compliance/Data residency: store only non-sensitive public links; keep PII minimal (display name optional). Cost: near-zero (static hosting free tier).
Users & personas: Learners (submitters): add their game title + URL + brief details during/after the session. Attendees (players): browse the grid and open/play games, optionally copy links. Facilitator (admin/light moderator): can clear/reset the board and optionally feature or sort entries.

Create a complete `project_spec.md` with:
...
> **Goal:** Smaller docs for deep dives, linked from the spec.
...
Anchor links back to `project_spec.md`. Keep each file pragmatic and implementation-ready.
```
...
9) Links/paths to the full docs (`project_spec.md`, `/docs/...`)
...
```text
From our project spec, extract a concise “Conventions & Guardrails” block I can paste into future coding prompts. Include:
...
## Prompt E — Diff‑Based Update of `project_spec.md` (use after sessions)
...
2) A unified diff (old → new) for `docs/project_spec.md`  
3) Update the `docs/project_spec.md`
...
*   After any substantial session, run **Prompt E** to update `project_spec.md`.
